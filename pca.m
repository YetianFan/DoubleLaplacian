function [mappedX, mapping] = pca(X, percent)
...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    cum_lam = cumsum(lambda);
    cum_lam = cum_lam/cum_lam(end);
    no_dims = sum(cum_lam < percent) + 1;
    
	M = M(:,ind(1:no_dims));
	
	% Apply mapping on the data
    if ~(size(X, 2) < size(X, 1))
        M = bsxfun(@times, X' * M, (1 ./ sqrt(size(X, 1) .* lambda(1:no_dims)))');     % normalize in order to get eigenvectors of covariance matrix
    end
    mappedX = X * M;
